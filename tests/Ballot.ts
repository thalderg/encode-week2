...
describe("Ballot", function () {
  ...
  describe("when the voter interact with the delegate function in the contract", function () {
    ...
    it("should transfer voting power", async () => {
      const [chairperson, voter1] = await ethers.getSigners();  
        ...
        // voter 1 delegate to voter1
        await ballotContract.delegate(voter1.address);
        const delegated = (await ballotContract.voters(chairperson.address)).voted;
        const voter1Weight = (await ballotContract.voters(voter1.address)).weight;
        expect(delegated).to.be.eq(true); 
        expect (voter1Weight).to.be.eq(2);
  });
});
  ...
  describe("when someone interact with the winningProposal function after one vote is cast for the first proposal", function () {
    ...
    it("should return 0", async () => {
      ...
      await expect (await ballotContract.winningProposal()).to.eq(0);
  });
});
